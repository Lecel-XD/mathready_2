<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>MATHREADY</title>
    <link rel="shortcut icon" href="#" type="image/x-icon">
    <link rel="stylesheet" href="css/fonts.css">
    <link rel="stylesheet" href="css/bootstrap-reboot.min.css">
    <link rel="stylesheet" href="css/bootstrap-grid.min.css">
    <link rel="stylesheet" href="sass/style.css">
</head>
<body>
    <?php 
        @include('nav.php');
     ?>
    <div class="container first_cont">
        <h1>Геометрия</h1>
        <h2>весь школьный курс</h2>
        <div class="theme_box">
            <h3 class="theme_text">Список тем</h3>
        </div>
        <div class="ol_box">
            <ol>
                <li><a href="">Планиметрия</a></li>
                <ol>
                    <li><a href="">Прямая, луч, отрезок, угол</a></li>
                    <li><a href="">Треугольник</a></li>
                    <li><a href="">Параллельные прямые</a></li>
                    <li><a href="">Окружность. Касательная, хорда, вписанная и описанная окружность</a></li>
                    <li><a href="">Симметрия</a></li>
                    <li><a href="">Четырёхугольник</a></li>
                    <li><a href="">Многоугольник</a></li>
                    <li><a href="">sin, cos, tg, ctg</a></li>
                </ol>
                <li><a href="">Стереометрия</a></li>
                <ol>
                    <li><a href="">Взаимное расположение прямых и плоскостей</a></li>
                    <li><a href="">Тетраэдр и параллелепипед</a></li>
                    <li><a href="">Двугранный, трёхгранный, многогранный угол</a></li>
                    <li><a href="">Многогранник</a></li>
                    <li><a href="">Пирамида</a></li>
                    <li><a href="">Цилиндр, конус и шар</a></li>
                    <li><a href="">Симметрия</a></li>
                    <li><a href=""></a></li>
                </ol>
                <li><a href="">Вектор</a></li>
                <li><a href="">Координаты в пространстве</a></li>
            </ol>
        </div>
    </div> 
</body>
</html>